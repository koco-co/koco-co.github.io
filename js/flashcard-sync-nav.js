(() => {
  ...
  function getGlobalSyncConfig() {
    try {
      return JSON.parse(
        document.getElementById("hfc-sync-config-data")?.textContent || "{}",
      );
    } catch (error) {
      return {};
    }
  ...
  function focusGiscus() {
    ...
    window.setTimeout(
      () => getGiscusFrame()?.focus({ preventScroll: true }),
      350,
    );
  }
  ...
  function refreshGiscusUi() {
    const nav = getNav();
    const commentStatus = nav?.querySelector(
      "[data-hfc-sync-nav-comment-status]",
    );
    const commentAction = nav?.querySelector("[data-hfc-sync-nav-giscus]");
    const hasFrame = Boolean(getGiscusFrame());
    if (hasFrame && giscusAuthState === "unknown")
      giscusAuthState = readGiscusAuthState();
    const commentLabel = {
    ...
    if (commentStatus) {
      ...
      commentStatus.hidden =
        !reviewAuthenticated || giscusAuthState === "unknown";
    }
    if (commentAction) {
      commentAction.hidden =
        !reviewAuthenticated ||
        giscusAuthState !== "unauthenticated" ||
        !hasFrame;
    }
  }

  function setGiscusAuthState(nextState) {
    if (!["authenticated", "unauthenticated", "unknown"].includes(nextState))
      return;
  ...
  function createNav() {
    ...
    nav
      .querySelector("[data-hfc-sync-nav-logout]")
    ...
    nav
      .querySelector("[data-hfc-sync-nav-giscus]")
      .addEventListener("click", () => {
        focusGiscus();
        popover.hidden = true;
        trigger.setAttribute("aria-expanded", "false");
  ...
  function renderNavState({
    ...
    trigger.setAttribute(
      "aria-label",
      authenticated ? "GitHub 账号与同步状态" : "使用 GitHub 登录",
    );
  ...
  function syncNavState() {
    ...
    const status =
      panel.querySelector("[data-hfc-sync-status]")?.textContent?.trim() ||
      "复习进度同步";
    const login = panel.querySelector("[data-hfc-login]");
